...
int compare_mtime(const void *a, const void *b) {
    return ((fileData *)b)->fileState.st_mtime - ((fileData *)a)->fileState.st_mtime;
}

int compare_atime(const void *a, const void *b) {
    return ((fileData *)b)->fileState.st_atime - ((fileData *)a)->fileState.st_atime;
}

int compare_ctime(const void *a, const void *b) {
    return ((fileData *)b)->fileState.st_ctime - ((fileData *)a)->fileState.st_ctime;
}
...
void traverse(int argc, char** argv) {
	...
	if(list_dir)
	{
		...
		for(int i = 0; i < argc; i++)
		{
			...
			if(stat(argv[i], &dirs[i].fileState) == -1)
			{
				perror("stat failed");
				continue;
			}
	...
	total_size = 0;
        ...
        while ((entry = readdir(dp)) != NULL) {
            ...
            if ((!no_sort && !list_all) && entry->d_name[0] == '.') continue;
//            if (list_dir && entry->d_type != DT_DIR) continue;
            ...
            if (stat(filepath, &files[cnt].fileState) == -1) {
                perror("stat failed");
                free(files[cnt].name);
                continue;
            }
...
void print_permissions(int mode) {
    ...
    // Special permissions
    if (mode & S_ISUID) str[2] = (mode & S_IXUSR) ? 's' : 'S'; // Setuid
    if (mode & S_ISGID) str[5] = (mode & S_IXGRP) ? 's' : 'S'; // Setgid
    if (mode & S_ISVTX) str[8] = (mode & S_IXOTH) ? 't' : 'T'; // Sticky bit
...
/*    char *columns_env = getenv("COLUMNS");
    int terminal_width = columns_env ? atoi(columns_env) : 80;  // Default to 80 if not set

    // Calculate how many columns can fit in the terminal width
    int spacing = max_width + 2;
    columns = terminal_width / spacing;
...
void print_long_format(fileData file) {
    ...
    printf("%2lu %-10s %-10s %5lld %s %s\n",
           file.fileState.st_nlink,
           pw ? pw->pw_name : "unknown",
           gr ? gr->gr_name : "unknown",
           (long long)file.fileState.st_size,
           timebuf,
           file.name);
}
